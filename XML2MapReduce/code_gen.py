...
#prasanth
def __gen_new_header__(fo):
    print >>fo, "package " + packagename + ";"
    print >>fo,"import java.io.BufferedReader;"
    print >>fo,"import java.io.File;"
    print >>fo,"import java.io.FileReader;"
    print >>fo,"import java.io.IOException;"
    print >>fo,"import java.util.ArrayList;"
    print >>fo,"import java.util.HashMap;"
    print >>fo,"\nimport org.apache.hadoop.conf.Configuration;"
    print >>fo,"import org.apache.hadoop.conf.Configured;"
    print >>fo,"import org.apache.hadoop.filecache.DistributedCache;"
    print >>fo,"import org.apache.hadoop.fs.FileStatus;"
    print >>fo,"import org.apache.hadoop.fs.FileSystem;"
    print >>fo,"import org.apache.hadoop.fs.Path;"
    print >>fo,"import org.apache.hadoop.io.LongWritable;"
    print >>fo,"import org.apache.hadoop.io.NullWritable;"
    print >>fo,"import org.apache.hadoop.io.Text;"
...
def __gen_header__(fo):
    ...
    print >>fo, "import org.apache.hadoop.mapreduce.lib.output.FileOutputFormat;"
    #prasanth - this package is not required
...
#prasanth
def __get_col_num__(key):
	start = key.find("[")
	if start == -1:
		print >>sys.stderr,"Error! Could not get column number of join key"
		print >>sys.stderr,"Internal Error:__get_col_num__"
		exit(29)
...
#prasanth
def __frjoin_gen_mr__(tree,left_name,fo):
    if tree.select_list is None:
        print >>sys.stderr,"Internal Error:__frjoin_gen_mr__"
        exit(29)
    ...
    reduce_key_type = "NullWritable"
    reduce_value_type = "Text"
    right_key_type_java = java_type_to_hash[right_key_type]
    left_key_type_java = java_type_to_hash[left_key_type]
    ...
    left_key_col_num = __get_col_num__(left_key)
    right_key_col_num = __get_col_num__(right_key)
    token_splits_left = int(left_key_col_num) + 2
    token_splits_right = int(right_key_col_num) + 2
    
    # generate map class
    print >>fo,"\tpublic static class Map extends  Mapper<LongWritable, Text, NullWritable, Text>{"
    print >>fo,"\t\tprivate HashMap<" + right_key_type_java + ", ArrayList<String>> ht = null;\n"
    print >>fo,"\t\tpublic void setup(Context context) throws IOException {\n"
    print >>fo,"\t\t\tht = new HashMap<" + right_key_type_java + ", ArrayList<String>>();"
    print >>fo,"\t\t\tPath[] paths = DistributedCache.getLocalCacheFiles(context.getConfiguration());"
    print >>fo,"\t\t\tif( paths == null) {"
    print >>fo,"\t\t\t\tthrow new IOException(\"Could not retrieve file from distributed cache.\");"
    print >>fo,"\t\t\t}"
    print >>fo,"\t\t\tfor(int i = 0; i<paths.length; i++) {"
    print >>fo,"\t\t\t\tSystem.out.println(\"Retrieving \"+paths[i]+\" from distributed cache.\");"
    print >>fo,"\t\t\t\tFile file = new File(paths[i].toString());"
    print >>fo,"\t\t\t\tBufferedReader br = null;"
    print >>fo,"\t\t\t\tString line = null;"
    print >>fo,"\t\t\t\ttry {"
    print >>fo,"\t\t\t\t\tbr = new BufferedReader(new FileReader(file));"
    print >>fo,"\t\t\t\t\twhile( (line = br.readLine()) != null ) {"
    print >>fo,"\t\t\t\t\t\tString record[] = line.split(\"\\\|\"," + str(token_splits_right) +");"
    if right_key_type_java == "Integer" :
    	print >>fo,"\t\t\t\t\t\tInteger key = Integer.parseInt(record[" + right_key_col_num + "]);"
    elif right_key_type_java == "Double" :
    	print >>fo,"\t\t\t\t\t\tDouble key = Double.parseDouble(record[" + right_key_col_num + "]);"
    else:
    	print >>fo,"\t\t\t\t\t\tString key = record[" + right_key_col_num + "];"
    	
    print >>fo,"\t\t\t\t\t\tif( ht.containsKey(key)) {"
    print >>fo,"\t\t\t\t\t\t\tht.get(key).add(line);"
    print >>fo,"\t\t\t\t\t\t}"
    print >>fo,"\t\t\t\t\t\telse {"
    print >>fo,"\t\t\t\t\t\t\tArrayList<String> values = new ArrayList<String>();"
    print >>fo,"\t\t\t\t\t\t\tvalues.add(line);"
    print >>fo,"\t\t\t\t\t\t\tht.put(key, values);"
    print >>fo,"\t\t\t\t\t\t}"
    print >>fo,"\t\t\t\t\t}"
    print >>fo,"\t\t\t\t}"
    print >>fo,"\t\t\t\tcatch(Exception e) {"
    print >>fo,"\t\t\t\t\te.printStackTrace();"
    print >>fo,"\t\t\t\t}"
    print >>fo,"\t\t\t}"
    print >>fo,"\t\t}\n"
    ...
    if tree.left_child.select_list is not None:
		if isinstance(tree.left_child,ystree.TableNode):
			for item in tree.left_child.select_list.tmp_exp_list:
				left_project_cols.append(item.column_name)
		else:
			for i in range(0,len(tree.left_child.select_list.tmp_exp_list)):
				left_project_cols.append(i)

    if tree.right_child.select_list is not None:
    	if isinstance(tree.right_child,ystree.TableNode):
    		for item in tree.right_child.select_list.tmp_exp_list:
    			right_project_cols.append(item.column_name)
    	else:
    		for i in range(0,len(tree.right_child.select_list.tmp_exp_list)):
				right_project_cols.append(i)
	
	# generate map function
    print >>fo,"\t\tpublic void map(LongWritable lineno, Text value, Context context)"
    print >>fo,"\t\t\t\tthrows IOException,InterruptedException {"
    print >>fo,"\t\t\tString[] leftRecord = value.toString().split(\"\\\|\"," + str(token_splits_left) +");"
    if left_key_type_java == "Integer" :
    	print >>fo,"\t\t\tInteger key = Integer.parseInt(leftRecord[" + left_key_col_num + "]);"
    elif left_key_type_java == "Double" :
    	print >>fo,"\t\t\tDouble key = Double.parseDouble(leftRecord[" + left_key_col_num + "]);"
    else:
    	print >>fo,"\t\t\tString key = leftRecord[" + left_key_col_num + "];"
    	
    print >>fo,"\t\t\tString[] leftcols = value.toString().split(\"\\\|\"," + str(max(left_project_cols)+2) + ");";
    print >>fo,"\t\t\tif( ht.containsKey(key)) {"
    print >>fo,"\t\t\t\tfor( String rightRecord : ht.get(key)) {"
    print >>fo,"\t\t\t\t\tString[] rightcols = rightRecord.split(\"\\\|\"," + str(max(right_project_cols)+2) + ");";
    print >>fo,"\t\t\t\t\tString output = new String();"
    
    rcount = 0
    lcount = 0
    for i in range(0,len(tree.select_list.tmp_exp_list)):
    	tabname = tree.select_list.tmp_exp_list[i].table_name
    	if tabname == "RIGHT":
    		print >>fo,"\t\t\t\t\toutput = output + rightcols[" + str(right_project_cols[rcount]) + "] + '|';"
    		rcount = rcount + 1
    	else:
    		print >>fo,"\t\t\t\t\toutput = output + leftcols[" + str(left_project_cols[lcount]) + "] + '|';"
    		lcount = lcount + 1

    print >>fo,"\t\t\t\t\tcontext.write(NullWritable.get(), new Text(output));"
    print >>fo,"\t\t\t\t}"
    print >>fo,"\t\t\t}"
    	
    if tree.join_explicit == True and join_type == "LEFT":
    	if tree.where_condition is None:
    		print >>fo,"\t\t\telse {"
    		print >>fo,"\t\t\t\tString ljoutput = new String();"
    		print >>fo,"\t\t\t\tString nulloutput = new String();"
    		
    		lcount = 0
    		for i in range(0,len(tree.select_list.tmp_exp_list)):
    			tabname = tree.select_list.tmp_exp_list[i].table_name
    			if tabname == "LEFT":
    				print >>fo,"\t\t\t\t\tljoutput = ljoutput + leftcols[" + str(left_project_cols[lcount]) + "] + '|';"
    				lcount = lcount + 1
    			else:
    				print >>fo,"\t\t\t\t\tljoutput = ljoutput + \"NULL|\";"
    		
    		print >>fo,"\t\t\t\tcontext.write(NullWritable.get(), new Text(ljoutput));"
    		print >>fo,"\t\t\t}"
    
    print >>fo,"\t\t}"
    print >>fo,"\t}\n"
    
    __gen_new_main__(tree,fo,map_key_type,map_value_type,reduce_key_type,reduce_value_type,True)
...
#prasanth    
def __gen_new_main__(tree,fo,map_key_type,map_value_type,reduce_key_type,reduce_value_type,reduce_bool):
	print >>fo,"\tpublic int run(String[] args) throws Exception {\n"
	...
	print >>fo,"\t\tConfiguration conf = new Configuration();\n"
	print >>fo,"\t\ttry {"
	print >>fo,"\t\t\tFileSystem fs = FileSystem.get(new Configuration());"
	print >>fo,"\t\t\tFileStatus[] status = fs.listStatus(new Path(" + arg[1] + "));"
	print >>fo,"\t\t\tfor (int i=0;i<status.length;i++) {"
	print >>fo,"\t\t\t\tSystem.out.println(\"Adding \"+status[i].getPath().toUri().toString()+\" to distributed cache.\");"
	print >>fo,"\t\t\t\tDistributedCache.addCacheFile(status[i].getPath().toUri(), conf);"
	print >>fo,"\t\t\t}"
	print >>fo,"\t\t}"
	print >>fo,"\t\tcatch (Exception e){"
	print >>fo,"\t\t\tthrow new Exception(\"Error adding files to distributed cache. File not found.\");"
	print >>fo,"\t\t}"
	print >>fo,"\t\tJob job = new Job(conf,\"" + jobname + "\");"
	print >>fo,"\t\tjob.setJarByClass(" + jobname + ".class);"
	print >>fo,"\t\tjob.setMapOutputKeyClass(NullWritable.class);"
	print >>fo,"\t\tjob.setMapOutputValueClass(Text.class);"
	print >>fo,"\t\tjob.setOutputKeyClass(NullWritable.class);"
	print >>fo,"\t\tjob.setOutputValueClass(Text.class);"
	print >>fo,"\t\tjob.setMapperClass(Map.class);"
	print >>fo,"\t\tFileInputFormat.addInputPath(job,new Path(" + arg[0] + "));"
	if arg[2] != None:
		print >>fo,"\t\tFileOutputFormat.setOutputPath(job, new Path(" + arg[2] + "));"
	print >>fo,"\t\treturn (job.waitForCompletion(true) ? 0 : 1);"
	print >>fo,"\t}\n"
...
def __join_code_gen__(tree,left_name,fo):
    ...
    if config.FRJoin == True and ( join_type == "LEFT" or join_type == "IMPLICIT" ):
    	print >>sys.stderr,"Generating code for Map Side Fragment Replicated Join..."
    	__gen_new_header__(fo)
    else:
    	if config.FRJoin == True:
    		print >>sys.stderr,"WARNING! FRJoin enabled and " + join_type + " OUTER JOIN specified which is not supported! "
    		print >>sys.stderr,"Falling back to generate reduce side " + join_type + " OUTER JOIN..."
    	__gen_header__(fo)
    
    print >>fo,"public class "+fo.name.split(".java")[0]+" extends Configured implements Tool{\n"

    if config.FRJoin == True and ( join_type == "LEFT" or join_type == "IMPLICIT" ):
		__frjoin_gen_mr__(tree,left_name,fo)
    else:
...
def compile_class(tree,codedir,package_path,filename,fo):
    ...
    #for hadoop 0.20.2
    cmd = "javac -classpath $HADOOP_HOME/hadoop-0.20.2-core.jar:$HADOOP_HOME/hadoop-0.20.2-tools.jar " 
    cmd += codedir + "/*.java -d ." 
    print >>fo,cmd
...
def ysmart_code_gen(argv,input_path,output_path):
    ...
    resultdir = "./result"
    #prasanth
    codedir = "edu/osu/cse/ysmart/" + config.queryname + "/"
    #codedir = "./YSmartCode"
    jardir = "./YSmartJar"
    ...
    if os.path.exists(jardir) is False:
    	os.makedirs(jardir)
